...
void mcpInit(MCP_t *dev, int16_t model, int16_t GPIO_MISO, int16_t GPIO_MOSI,
             int16_t GPIO_SCLK, int16_t GPIO_CS, int16_t input) {
  esp_err_t ret;
  ...
  spi_bus_config_t buscfg = {.sclk_io_num = GPIO_SCLK,
...
int16_t mcpReadData(MCP_t *dev, int16_t channel) {
  ...
  if (dev->_model == MCP3002 || dev->_model == MCP3202) {
    ...
    // [OUT] -- -- -- ----- -------- -------- ---- 00 B9 B8 B7 B6 B5 B4 B3 B2 B1
    // B0 MCP3202 [IN]  00 00 00 START SGL/DIFF ODD/SIGN MSBF [OUT] -- -- --
  ...
  } else if (dev->_model == MCP3004 || dev->_model == MCP3008 ||
             dev->_model == MCP3204 || dev->_model == MCP3208 ||
             dev->_model == MCP3302 || dev->_model == MCP3304) {
    // Need DMY bit
    // MCP300x
    ...
    // MCP320x
    // [IN]  00 START SGL/DIFF D2 D1 D0 DMY
    // [OUT] -- ----- -------- -- -- -- --- 00 B11 B10 B9 B8 B7 B6 B5 B4 B3 B2
...
#endif
  assert(ret == ESP_OK);
  ESP_LOGD(TAG, "rbuf[0]=%02X rbuf[1]=%02X rbuf[2]=%02X", rbuf[0], rbuf[1],
           rbuf[2]);
